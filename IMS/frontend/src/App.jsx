...
function CustomerView() {
  ...
  return (
    <div className="min-h-screen bg-[#f9f6f0] font-sans text-stone-800">
      {/* Customer Navbar */}
      <nav className="bg-white border-b border-stone-200 sticky top-0 z-30 shadow-sm">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 flex justify-between items-center h-16">
          ...
          <Link to="/login" className="text-xs font-bold text-stone-500 hover:text-pink-600 transition uppercase tracking-wider bg-stone-100 px-4 py-2 rounded-lg">
            Admin Login
          </Link>
        </div>
      </nav>
        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {filteredGarments.map(item => (
              <div key={item.id} className="bg-white rounded-3xl shadow-sm border border-stone-200/80 overflow-hidden flex flex-col group relative">
                {item.total_pieces === 0 && (
                ...
                <div className="relative h-72 bg-[#f2ece4] overflow-hidden flex items-center justify-center p-4">
                  {item.image ? (
                    <img src={item.image} alt={item.name} className={`w-full h-full object-cover rounded-xl transition duration-500 ${item.total_pieces === 0 ? 'opacity-50 grayscale' : 'group-hover:scale-105'}`} />
                  ) : (
...
// ==========================================
// 3. ADMIN DASHBOARD (YOUR EXISTING POS)
// ==========================================
  ...
  return (
    <div className="min-h-screen bg-[#f9f6f0] text-stone-800 flex flex-col md:flex-row relative font-sans">
      ...
      <main className="flex-1 p-4 sm:p-6 md:p-8 overflow-y-auto min-w-0">
              ...
              {filteredGarments.map((item) => (
                <div key={item.id} onClick={() => openProductModal(item, 'sell')} className="bg-white rounded-3xl shadow-sm hover:shadow-xl border border-stone-200/80 transition duration-300 overflow-hidden flex flex-col group cursor-pointer relative">
                  <div className="relative h-64 bg-[#f2ece4] overflow-hidden flex items-center justify-center pt-2">
                    {item.image ? (<img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition" />) : (<span className="text-6xl text-stone-300">👗</span>)}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                  {garments.filter(g => (g.batch_name || 'Uncategorized') === selectedBatch).map((item) => (
                    <div key={item.id} className="bg-white rounded-3xl shadow-sm border border-stone-200 overflow-hidden flex flex-col relative group">
                      <div onClick={() => openProductModal(item, 'sell')} className="relative h-64 bg-[#f2ece4] flex items-center justify-center cursor-pointer">
                        {item.image ? (<img src={item.image} alt={item.name} className="w-full h-full object-cover group-hover:scale-105 transition" />) : (<span className="text-6xl text-stone-300">👗</span>)}
        ...
        {/* HISTORY & ANALYTICS & PREORDERS... (Simplified visually here to save space, but keeping full features) */}
        {activeTab === 'history' && (
